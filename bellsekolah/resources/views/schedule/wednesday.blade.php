      ...
      <div class="container-fluid py-1 px-3">
        <nav aria-label="breadcrumb">
          <ol class="breadcrumb bg-transparent mb-0 pb-0 pt-1 px-0 me-sm-6 me-5">
            ...
            <li class="breadcrumb-item text-sm text-dark active" aria-current="page">Wednesday Schedules</li>
          </ol>
          <h6 class="font-weight-bolder mb-0">Wednesday Schedules</h6>
        </nav>
                  ...
                  <div class="card-body px-0 pb-2">
                      <div class="table-responsive p-0">
                          <table class="table align-items-center mb-0">
                              ...
                              <tbody>
                                  @foreach($wednesday as $wd)
                                  <tr>
                                      <td class="text-uppercase text-secondary text-xxs font-weight-bolder align-middle">{{ $loop->index+1 }}</td>
                                      <td class="text-uppercase text-secondary text-xxs font-weight-bolder align-middle">{{ $wd->hari }}</td>
                                      <td class="text-uppercase text-secondary text-xxs font-weight-bolder ps-2 align-middle">{{ date('H:i', strtotime($wd->jam)) }}</td>
                                      <td class="text-center text-uppercase text-secondary text-xxs font-weight-bolder align-middle">{{ $wd->jadwal }}</td>
                                      <td class="text-center text-uppercase text-secondary text-xxs font-weight-bolder align-middle">{{ $wd->audio }}</td>
                                      <td align="center" class="align-middle">
                                          <div class="btn-group" role="group" aria-label="Aksi">
                                              <!-- Tombol Edit -->
                                              <button class="btn btn-sm btn-info mx-1 rounded-pill edit-button" data-toggle="modal" data-target="#editModal-{{ $wd->id }}">
                                                  <i class="fas fa-edit"></i> Edit
                                              </button>
  
                                              <!-- Tombol Delete -->
                                              <form action="{{ route('wednesday.destroy', ['wednesday' => $wd->id]) }}" method="post">
                  ...
                  <div class="row">
                      <div class="col-md-6">
                          ...
                          <div class="mb-3">
                              ...
                              <input type="text" class="form-control" id="hari-add" name="hari" value="Wednesday" readonly>
                              @error('hari')
...
<!-- End Modal Add -->
    ...
    <!-- Start Modal Edit -->
    <div class="modal fade" id="editModal-{{ $wed->id }}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header bg-primary text-white">
                ...
                <input type="hidden" id="audioPath-{{ $wed->id }}" value="{{ asset('storage/' . $wed->audio) }}">

                <form action="{{ route('wednesday.update', ['wednesday' => $wed->id]) }}" method="POST" enctype="multipart/form-data">
                    <div class="modal-body">
                        @method('PUT')
                        @csrf
                        <div class="row">
                            <div class="col-md-6">
                                <input type="hidden" value="{{$wed->id}}" id="tue_id">

                                {{-- Hari --}}
                                <div class="mb-3">
                                    <label for="hari-edit" class="form-label"><i class="fas fa-calendar"></i> Hari</label>
                                    <input type="text" class="form-control" id="hari-edit" name="hari" value="{{ $wed->hari }}" readonly>
                                    @error('hari')
                                        <div class="alert alert-danger mt-2" role="alert">{{ $message }}</div>
                                    @enderror
                                </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="time" class="form-control @error('jam') is-invalid @enderror" value="{{ $wed->jam }}" id="jam-edit" name="jam">
                                    @error('jam')
                                        <div class="alert alert-danger mt-2" role="alert">
                                            {{ $message }}
                                        </div>
                            ...
                            <div class="col-md-6">
                                ...
                                <div class="mb-3">
                                    ...
                                    <input type="text" class="form-control @error('jadwal') is-invalid @enderror" id="jadwal-edit" name="jadwal" value="{{ $wed->jadwal }}" placeholder="Jadwal">
                                    @error('jadwal')
                                        <div class="alert alert-danger mt-2" role="alert">{{ $message }}</div>
                                    @enderror
                                </div>
                                ...
                                <div class="mb-3">
                                  ...
                                  <audio controls id="audioPreview-{{ $wed->id }}" class="mb-3" style="display: none;">
                                      @if($wed->audio)
                                      <source src="{{ asset('storage/audio_wednesday/' . $wed->audio) }}" type="audio/*">
                                      @endif
                                  </audio>
                                  <input type="file" class="form-control @error('audio') is-invalid @enderror" id="audio-edit-{{ $wed->id }}" name="audio" accept="audio/*" onchange="previewAudio('{{ $wed->id }}')">
                                  @error('audio')
                                      <div class="alert alert-danger mt-2" role="alert">{{ $message }}</div>
                                  @enderror
                              </div>
