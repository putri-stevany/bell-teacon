      ...
      <div class="container-fluid py-1 px-3">
        <nav aria-label="breadcrumb">
          <ol class="breadcrumb bg-transparent mb-0 pb-0 pt-1 px-0 me-sm-6 me-5">
            ...
            <li class="breadcrumb-item text-sm text-dark active" aria-current="page">Monday Schedules</li>
          </ol>
          <h6 class="font-weight-bolder mb-0">Monday Schedules</h6>
        </nav>
                  ...
                  <div class="row">
                      <div class="col-md-6">
                          ...
                          <div class="mb-3">
                              ...
                              <input type="text" class="form-control" id="hari-add" name="hari" value="Monday" readonly>
                              @error('hari')
...
<!-- End Modal Add -->
    ...
    <!-- Start Modal Edit -->
    <div class="modal fade" id="editModal-{{ $mon->id }}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header bg-primary text-white">
                ...
                <input type="hidden" id="audioPath-{{ $mon->id }}" value="{{ asset('storage/' . $mon->audio) }}">

                <form action="{{ route('monday.update', ['monday' => $mon->id]) }}" method="POST" enctype="multipart/form-data">
                    <div class="modal-body">
                        @method('PUT')
                        @csrf
                        <div class="row">
                            <div class="col-md-6">
                                <input type="hidden" value="{{$mon->id}}" id="mon_id">

                                {{-- Hari --}}
                                <div class="mb-3">
                                    <label for="hari-edit" class="form-label"><i class="fas fa-calendar"></i> Hari</label>
                                    <input type="text" class="form-control" id="hari-edit" name="hari" value="{{ $mon->hari }}" readonly>
                                    @error('hari')
                                        <div class="alert alert-danger mt-2" role="alert">{{ $message }}</div>
                                    @enderror
                                </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="time" class="form-control @error('jam') is-invalid @enderror" value="{{ $mon->jam }}" id="jam-edit" name="jam">
                                    @error('jam')
                                        <div class="alert alert-danger mt-2" role="alert">
                                            {{ $message }}
                                        </div>
                            ...
                            <div class="col-md-6">
                                ...
                                <div class="mb-3">
                                    ...
                                    <input type="text" class="form-control @error('jadwal') is-invalid @enderror" id="jadwal-edit" name="jadwal" value="{{ $mon->jadwal }}" placeholder="Jadwal">
                                    @error('jadwal')
                                        <div class="alert alert-danger mt-2" role="alert">{{ $message }}</div>
                                    @enderror
                                </div>
                                ...
                                <div class="mb-3">
                                  ...
                                  <audio controls id="audioPreview-{{ $mon->id }}" class="mb-3" style="display: none;">
                                      @if($mon->audio)
                                      <source src="{{ asset('storage/audio_monday/' . $mon->audio) }}" type="audio/*">
                                      @endif
                                  </audio>
                                  <input type="file" class="form-control @error('audio') is-invalid @enderror" id="audio-edit-{{ $mon->id }}" name="audio" accept="audio/*" onchange="previewAudio('{{ $mon->id }}')">
                                  @error('audio')
                                      <div class="alert alert-danger mt-2" role="alert">{{ $message }}</div>
                                  @enderror
                              </div>
